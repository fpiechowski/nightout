...
final profileByIdInProvider =
  ...
  return await Future.wait(documentList.documents.map((e) async => Profile.fromDocument(e, ref)));
});
...
final profileByPhoneNumberProvider =
  ...
  final document = documentList.documents.firstOrNull;

  Profile? profile;

  if (document != null) {
    profile = await Profile.fromDocument(document, ref);
  }
